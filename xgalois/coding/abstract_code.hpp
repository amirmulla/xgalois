...
template <typename ElementType> class Encoder;
template <typename ElementType> class Decoder;
...
public:
    ...
    using encoder_factory_type = std::function<std::unique_ptr<Encoder<ElementType>>(const AbstractCode<ElementType>*)>;
    using decoder_factory_type = std::function<std::unique_ptr<Decoder<ElementType>>(const AbstractCode<ElementType>*)>;

    // Constructor
    AbstractCode(size_t length,
    ...
    std::unique_ptr<Encoder<ElementType>> GetEncoder(const std::string& name = "") const;
    std::unique_ptr<Decoder<ElementType>> GetDecoder(const std::string& name = "") const;
...
protected:
    ...
    mutable std::unordered_map<std::string, std::unique_ptr<Encoder<ElementType>>> encoder_cache_;
    mutable std::unordered_map<std::string, std::unique_ptr<Decoder<ElementType>>> decoder_cache_;
};
...
AbstractCode<ElementType>::DecodeToMessage(const codeword_type& received_word,
                                          const std::string& decoder_name) const {
...
template <typename ElementType>
std::unique_ptr<Encoder<ElementType>>
AbstractCode<ElementType>::GetEncoder(const std::string& name) const {
    std::string encoder_name = name.empty() ? default_encoder_name_ : name;

    if (encoder_name.empty()) {
        throw std::runtime_error("No encoder name specified and no default encoder set");
    }
...
template <typename ElementType>
std::unique_ptr<Decoder<ElementType>>
AbstractCode<ElementType>::GetDecoder(const std::string& name) const {
    std::string decoder_name = name.empty() ? default_decoder_name_ : name;

    if (decoder_name.empty()) {
        throw std::runtime_error("No decoder name specified and no default decoder set");
    }
    ...
    switch (metric_) {
        case Metric::HAMMING:
            for (size_t i = 0; i < word.size(); ++i) {
                if (word(i) != ElementType{}) { // Assuming zero element is default constructed
                    weight++;
                }
            }
            break;
