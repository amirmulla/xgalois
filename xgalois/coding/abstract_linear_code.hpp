...
public:
    ...
    // Constructor
    AbstractLinearCode(size_t length,
                      size_t dimension,
        ...
        : AbstractCode<ElementType>(length, default_encoder_name, default_decoder_name, metric),
    ...
    // Information rate
    double Rate() const { return static_cast<double>(dimension_) / this->length_; }

    // Redundancy
    size_t Redundancy() const { return this->length_ - dimension_; }

    // Linear combination of codewords
    virtual codeword_type LinearCombination(const std::vector<codeword_type>& codewords,
...
protected:
    ...
    matrix_type ComputeParityCheckFromGenerator(const matrix_type& generator) const;
    matrix_type ComputeGeneratorFromParityCheck(const matrix_type& parity_check) const;
};
...
template <typename ElementType>
bool AbstractLinearCode<ElementType>::Contains(const codeword_type& word) const {
...
template <typename ElementType>
typename AbstractLinearCode<ElementType>::codeword_type
AbstractLinearCode<ElementType>::LinearCombination(const std::vector<codeword_type>& codewords,
                                                  const xt::xarray<ElementType>& coefficients) const {
    if (codewords.size() != coefficients.size()) {
        throw std::invalid_argument("Number of codewords must match number of coefficients");
    }
...
AbstractLinearCode<ElementType>::ComputeParityCheckFromGenerator(const matrix_type& generator) const {
    // For a generator matrix G of size k x n, the parity check matrix H is (n-k) x n
    // such that G * H^T = 0
    ...
    if (k >= n) {
        throw std::invalid_argument("Generator matrix must have more columns than rows");
    }

    // For systematic form [I_k | P], H = [-P^T | I_{n-k}]
    // This is a simplified approach - real implementation would handle general case
    matrix_type H(n - k, n);
...
template <typename ElementType>
typename AbstractLinearCode<ElementType>::matrix_type
AbstractLinearCode<ElementType>::ComputeGeneratorFromParityCheck(const matrix_type& parity_check) const {
    // This is the dual operation - compute generator from parity check
    // For a parity check matrix H of size (n-k) x n, find generator G of size k x n
    // such that G * H^T = 0
