...
public:
    ...
    explicit GRSEncoder(const AbstractCode<ElementType>* code)
        ...
        if (!grs_code_) {
            throw std::invalid_argument("GRSEncoder requires a GeneralizedReedSolomonCode");
        }
    }
    ...
    size_t MessageLength() const override {
        return grs_code_->Dimension();
    }

    std::string ToString() const override {
        return "GRS Encoder for [" +
    ...
    codeword_type SystematicEncode(const message_type& message) const {
        ...
        // such that f(α_0) = m_0/v_0, f(α_1) = m_1/v_1, ..., f(α_{k-1}) = m_{k-1}/v_{k-1}
            ...
            for (size_t j = 0; j < n; ++j) {
                if (i != j) {
                    ...
                    for (size_t k = 0; k < basis_coeffs.size(); ++k) {
                        new_coeffs(k) = field->Add(new_coeffs(k),
                                                  field->Mul(basis_coeffs(k),
                                                            field->Neg(points(j))));
