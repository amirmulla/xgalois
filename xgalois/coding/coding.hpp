...
CreateReedSolomonCode(std::shared_ptr<GaloisFieldBase<ElementType>> field,
                     size_t length,
                     size_t dimension,
                     const ElementType& primitive_element) {
    return GeneralizedReedSolomonCode<ElementType>::ReedSolomon(field, length, dimension, primitive_element);
}
...
template <typename ElementType>
std::unique_ptr<GeneralizedReedSolomonCode<ElementType>>
CreateGeneralizedReedSolomonCode(std::shared_ptr<GaloisFieldBase<ElementType>> field,
...
template <typename ElementType>
std::unique_ptr<CyclicCode<ElementType>>
CreateCyclicCode(std::shared_ptr<GaloisFieldBase<ElementType>> field,
                size_t length,
                const xg::poly::PolyDense<ElementType>& generator_poly) {
    return std::make_unique<CyclicCode<ElementType>>(field, length, generator_poly);
}
...
bool SatisfiesHammingBound(const AbstractLinearCode<ElementType>& code) {
    ...
    return static_cast<size_t>(std::pow(q, k)) * volume <= static_cast<size_t>(std::pow(q, n));
}
