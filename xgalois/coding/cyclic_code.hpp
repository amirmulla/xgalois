...
public:
    ...
    CyclicCode(std::shared_ptr<GaloisFieldBase<ElementType>> field,
               size_t length,
               const polynomial_type& generator_poly)
        : AbstractLinearCode<ElementType>(length,
                                         length - generator_poly.Degree(),
                                         "CyclicEncoder",
                                         "CyclicSyndrome",
                                         Metric::HAMMING),
          field_(field), generator_poly_(generator_poly) {

        if (generator_poly.Degree() >= length) {
            throw std::invalid_argument("Generator polynomial degree must be less than code length");
        }
    ...
    static std::unique_ptr<CyclicCode<ElementType>>
    FromParityCheckPolynomial(std::shared_ptr<GaloisFieldBase<ElementType>> field,
                             size_t length,
                             const polynomial_type& parity_check_poly) {
        auto generator_poly = ComputeGeneratorFromParityCheck(field, length, parity_check_poly);
    ...
    std::unique_ptr<AbstractLinearCode<ElementType>> DualCode() const override {
        ...
        return std::make_unique<CyclicCode>(field_, this->length_, parity_check_poly_);
    }
    ...
    const polynomial_type& ParityCheckPolynomial() const { return parity_check_poly_; }
    ...
    codeword_type CyclicShift(const codeword_type& codeword, int shift = 1) const {
...
private:
    ...
    polynomial_type ComputeParityCheckPolynomial() const {
        ...
        std::vector<ElementType> xn_minus_1_coeffs(this->length_ + 1, ElementType{});
...
template <typename ElementType> class CyclicEncoder;
template <typename ElementType> class CyclicSyndromeDecoder;
