...
private:
    ...
    std::unordered_map<std::vector<ElementType>, std::vector<ElementType>> syndrome_table_;
    ...
    void GenerateErrorCombinations(size_t n, size_t q, size_t remaining_weight,
                                  size_t start_pos, std::vector<size_t>& positions,
                                  const matrix_type& parity_check) {
        if (remaining_weight == 0) {
            // Generate all non-zero field elements for these positions
            GenerateFieldCombinations(n, q, positions, 0,
                                    std::vector<ElementType>(), parity_check);
            return;
        }

        for (size_t pos = start_pos; pos <= n - remaining_weight; ++pos) {
            positions.push_back(pos);
            GenerateErrorCombinations(n, q, remaining_weight - 1, pos + 1,
                                    positions, parity_check);
            positions.pop_back();
        }
    }
    ...
    void GenerateFieldCombinations(size_t n, size_t q,
        ...
        for (size_t i = 1; i < q; ++i) {
            ...
            GenerateFieldCombinations(n, q, positions, pos_index + 1,
                                    field_values, parity_check);
            field_values.pop_back();
        }
    }

    std::vector<ElementType> ComputeSyndrome(const std::vector<ElementType>& error_pattern,
    ...
    std::vector<ElementType> LookupErrorPattern(const vector_type& syndrome) const {
