...
public:
    ...
    explicit GRSDecoder(const AbstractCode<ElementType>* code)
        ...
        grs_code_ = dynamic_cast<const GeneralizedReedSolomonCode<ElementType>*>(code);
        if (!grs_code_) {
            throw std::invalid_argument("GRSDecoder requires a GeneralizedReedSolomonCode");
        }
    ...
    codeword_type DecodeToCode(const codeword_type& received_word) const override {
        if (received_word.size() != this->code_->Length()) {
            throw std::invalid_argument("Received word length must match code length");
        }
        ...
        auto error_values = FindErrorValues(syndrome, error_locator, error_locations);
    ...
    // Decode to message
    message_type DecodeToMessage(const codeword_type& received_word) const override {
...
private:
    ...
    std::vector<ElementType> ComputeSyndrome(const codeword_type& received_word) const {
    ...
    polynomial_type FindErrorLocator(const std::vector<ElementType>& syndrome) const {
        auto field = grs_code_->Field();
        ...
        for (size_t t = 1; t <= max_errors_; ++t) {
            ...
            // S * Lambda = -S_shifted
            std::vector<std::vector<ElementType>> matrix(t, std::vector<ElementType>(t));
    ...
    std::vector<size_t> FindErrorLocations(const polynomial_type& error_locator) const {
        auto field = grs_code_->Field();
        auto eval_points = grs_code_->EvaluationPoints();
    ...
    std::vector<ElementType> FindErrorValues(const std::vector<ElementType>& syndrome,
        ...
        for (size_t i = 0; i < error_locations.size(); ++i) {
            ...
            if (denominator == ElementType{}) {
                throw std::runtime_error("Error in Forney's algorithm: zero denominator");
            }
    ...
    // Compute error evaluator polynomial
    polynomial_type ComputeErrorEvaluator(const std::vector<ElementType>& syndrome,
                                         const polynomial_type& error_locator) const {
        auto field = grs_code_->Field();

        // Create syndrome polynomial
        std::vector<ElementType> syndrome_coeffs(syndrome.size() + 1, ElementType{});
    ...
    codeword_type CorrectErrors(const codeword_type& received_word,
    ...
    std::vector<ElementType> SolveLinearSystem(const std::vector<std::vector<ElementType>>& A,
        ...
        std::vector<std::vector<ElementType>> augmented(n, std::vector<ElementType>(n + 1));
