...
class GaloisFieldPrime : public GaloisFieldBase<ElementType> {
 ...
 public:
  ...
  uint32_t Log(const ElementType &a,
    ...
    if (a == this->MultiplicativeIdentity()) {
      return 0;
    }

    uint64_t phi = this->Order() - 1;
    if (phi == 0) {
...
class GaloisFieldPrimeTable : public GaloisFieldPrime<ElementType> {
 ...
 public:
  ...
  inline ElementType Mul(const ElementType &a,
    ...
    if (this->p_ == 2) {
      return GaloisFieldPrime<ElementType>::Mul(a, b);
    }
  ...
  inline ElementType Div(const ElementType &a,
    ...
    if (this->p_ == 2) {
      return GaloisFieldPrime<ElementType>::Div(a, b);
    }
    if (a >= this->p_ || b >= this->p_) {
  ...
  inline ElementType Inv(const ElementType &a) const override {
    ...
    if (this->p_ == 2) {
      return GaloisFieldPrime<ElementType>::Inv(a);
    }
    if (a >= this->p_) {
 ...
 private:
  std::vector<ElementType> alpha_pow_;
  std::vector<ElementType> log_alpha_;
};
