...
private:
    static inline const std::string db_path_ = "xgalois/databases/prime_factors.db";

protected:
    const std::string& get_db_path() const override {
        return db_path_;
    }
...
public:
    ...
    PrimeFactorsResult fetch(long long n) const {
        ...
        while (std::getline(db_file, line)) {
            ...
            while(std::getline(ss, segment, ',')) { // Fixed: Changed \\\',\\\' to \',\'
            ...
            if (current_n == n) {
                ...
                if (segments.size() < 3) {
                     std::cerr << "Skipping malformed line (segment count): " << line << std::endl;
                    continue;
                }

                std::stringstream factors_ss(segments[1]);
                std::string factor_pair_str;
                while (std::getline(factors_ss, factor_pair_str, ';')) { // Fixed: Changed \\\';\\\' to \';\'
                    std::stringstream factor_pair_ss(factor_pair_str);
                    std::string factor_str, mult_str;
                    if (std::getline(factor_pair_ss, factor_str, ':') && std::getline(factor_pair_ss, mult_str, ':')) { // Fixed: Changed \\\':\\\' to \':\' (twice)
                        try {
                            result.factors.push_back(std::stoll(factor_str));
                            result.multiplicities.push_back(std::stoi(mult_str));
                        } catch (const std::invalid_argument& ia) {
                            std::cerr << "Skipping malformed factor/multiplicity: " << factor_pair_str << std::endl;
                ...
                try {
                    result.composite = std::stoi(segments[2]);
                     ...
                     std::cerr << "Skipping malformed line (composite flag): " << line << std::endl;
                     result.composite = -1;
                }
                return result;
            }
        }
        throw std::runtime_error("Number not found in prime factors database: " + std::to_string(n));
    }
};

// ----------------- Irreducible Polynomial Database -----------------
// This database stores irreducible polynomial coefficients and degrees for finite fields.
// Each entry is expected to be in the format:
...
private:
    static inline const std::string db_path_ = "xgalois/databases/irreducible_polys.db";
    ...
    IrreduciblePolyResult fetch(int characteristic, int degree) const {
        ...
        while (std::getline(db_file, line)) {
            ...
            try {
                ...
                std::cerr << "Skipping malformed line (char/deg): " << line << std::endl;
                continue;
            }
            ...
            if (current_char == characteristic && current_deg == degree) {
                ...
                std::string deg_str;
                while (std::getline(degrees_ss, deg_str, ';')) { // Fixed: Changed \\\';\\\' to \';\'
                    try {
                ...
                while (std::getline(coeffs_ss, coeff_str, ';')) { // Fixed: Changed \\\';\\\' to \';\'
                    try {
                        result.nonzero_coeffs.push_back(std::stoi(coeff_str));
                    } catch (const std::invalid_argument& ia) {
                        std::cerr << "Skipping malformed coefficient: " << coeff_str << std::endl;
                ...
                if (result.nonzero_degrees.size() != result.nonzero_coeffs.size()) {
                    std::cerr << "Mismatch between degree and coefficient counts: " << line << std::endl;
                    continue;
                }
                return result;
            }
        }
        throw std::runtime_error("Irreducible polynomial not found for characteristic " + std::to_string(characteristic) + ", degree " + std::to_string(degree));
    }
};
...
private:
    static inline const std::string db_path_ = "xgalois/databases/conway_polys.db";
...
public:
    ...
    // Example: 2,4,"4;1;0","1;1;1" (for x^4 + x + 1 over GF(2) - a Conway polynomial for GF(2^4))
    ...
    ConwayPolyResult fetch(int characteristic, int degree) const {
        ...
        throw std::runtime_error("Conway polynomial not found for characteristic " + std::to_string(characteristic) + ", degree " + std::to_string(degree));
    }
};
