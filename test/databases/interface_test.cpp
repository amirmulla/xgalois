...
protected:
    ...
    void TestPrimeFactors(const PrimeTestCase& test_case) {
        ...
        EXPECT_EQ(result.multiplicities.size(), test_case.expected_multiplicities.size());
    ...
    void TestPolynomialFetch(const PolyTestCase& test_case, bool is_irreducible = true) {
        if (is_irreducible) {
            IrreduciblePolyDatabase db;
            IrreduciblePolyResult result = db.fetch(test_case.characteristic, test_case.degree);

            EXPECT_EQ(result.nonzero_degrees.size(), test_case.expected_degrees.size());
            ...
            ConwayPolyResult result = db.fetch(test_case.characteristic, test_case.degree);
...
TEST_F(DatabaseInterfaceTest, PrimeFactorsMultipleFactorsWithHigherMultiplicity) {
...
TEST_F(DatabaseInterfaceTest, DatabasePerformance) {
    ...
    // With the large database, this might take longer, so we set a reasonable threshold
...
TEST_F(DatabaseInterfaceTest, DatabaseDataConsistency) {
    ...
    EXPECT_EQ(irreducible_result.nonzero_degrees.size(), irreducible_result.nonzero_coeffs.size());
    EXPECT_FALSE(irreducible_result.nonzero_degrees.empty());

    ConwayPolyResult conway_result = conway_db.fetch(2, 2);
    EXPECT_EQ(conway_result.nonzero_degrees.size(), conway_result.nonzero_coeffs.size());
...
TEST_F(DatabaseInterfaceTest, MultipleDatabaseOperations) {
    ...
    // Test that we can successfully perform multiple operations on different databases

    // Prime factors operations
    PrimeFactorsResult prime1 = prime_db.fetch(273323);
    PrimeFactorsResult prime2 = prime_db.fetch(21218);
    EXPECT_NE(prime1.composite, prime2.composite);  // One is prime, one is composite
    ...
    EXPECT_NE(irreducible1.nonzero_degrees[0], irreducible2.nonzero_degrees[0]);  // Different leading degree
...
TEST_F(DatabaseInterfaceTest, ConvertPolyResultToStringMismatch) {
    ...
    EXPECT_THROW(ConvertPolyResultToString(degrees, coeffs, 2), std::invalid_argument);
}
...
class PrimeFactorsParameterizedTest : public DatabaseInterfaceTest,
...
INSTANTIATE_TEST_SUITE_P(
    AllPrimeFactorizations,
    PrimeFactorsParameterizedTest,
    ::testing::ValuesIn(kPrimeTestCases)
);

// Parameterized test for irreducible polynomials
class IrreduciblePolyParameterizedTest : public DatabaseInterfaceTest,
...
class ConwayPolyParameterizedTest : public DatabaseInterfaceTest,
                                   public ::testing::WithParamInterface<PolyTestCase> {
};
...
TEST_F(DatabaseInterfaceTest, PolynomialEdgeCases) {
    ...
    for (const auto& [char_val, deg_val] : invalid_params) {
        EXPECT_THROW(irreducible_db.fetch(char_val, deg_val), std::runtime_error)
            << "Expected exception for characteristic=" << char_val << ", degree=" << deg_val;
...
TEST_F(DatabaseInterfaceTest, StressTestMultipleLookups) {
    ...
    for (int i = 0; i < 10; ++i) {
        ...
        for (const auto& test_case : kIrreducibleTestCases) {
            IrreduciblePolyResult result = irreducible_db.fetch(
                test_case.characteristic, test_case.degree);
            EXPECT_FALSE(result.nonzero_degrees.empty());
        }

        for (const auto& test_case : kConwayTestCases) {
            ConwayPolyResult result = conway_db.fetch(
                test_case.characteristic, test_case.degree);
            EXPECT_FALSE(result.nonzero_degrees.empty());
        }
    }
    ...
    EXPECT_LT(duration.count(), 10000) << "Stress test took too long: " << duration.count() << "ms";
}

TEST_F(DatabaseInterfaceTest, ConcurrentDatabaseAccess) {
    // Test that multiple databases can be used simultaneously without interference
    ...
    EXPECT_EQ(irreducible_result1.nonzero_degrees, irreducible_result2.nonzero_degrees);
    EXPECT_EQ(irreducible_result1.nonzero_coeffs, irreducible_result2.nonzero_coeffs);
