...
TEST_F(PolynomialDenseTest, Subtraction) {
  ...
  // p1 - p2 = (1-4) + (2-5)x + 3x^2 = -3 + (-3)x + 3x^2 = 4 + 4x + 3x^2 (in
  // GF(7))
  auto result = p1 - p2;
...
TEST_F(PolynomialDenseTest, Multiplication) {
  ...
  // p1 * p2 = (1 + 2x)(3 + 4x) = 3 + 4x + 6x + 8x^2 = 3 + 10x + 8x^2 = 3 + 3x +
  // x^2 (in GF(7))
  auto result = p1 * p2;
...
TEST_F(PolynomialDenseTest, BinaryFieldOperations) {
  ...
  std::vector<GaloisFieldElementBase<GaloisFieldBinary>> coeffs1 = {
      gf2_one, gf2_zero, gf2_one};  // 1 + x^2
  PolynomialDense<GaloisFieldBinary> poly1(coeffs1);

  std::vector<GaloisFieldElementBase<GaloisFieldBinary>> coeffs2 = {
      gf2_one, gf2_one};  // 1 + x
