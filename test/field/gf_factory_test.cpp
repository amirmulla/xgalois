...
TEST_F(GaloisFieldFactoryTest, PerformanceTest) {
  ...
  EXPECT_LT(duration.count(), 30000);  // Should complete in less than 30 seconds
...
TEST_F(GaloisFieldFactoryTest, PrimeFactorization) {
  ...
  EXPECT_NO_THROW(GF(9, "poly"));    // 3^2 - general extension field (requires poly representation)
  EXPECT_NO_THROW(GF(25, "poly"));   // 5^2 - general extension field (requires poly representation)
  EXPECT_NO_THROW(GF(27, "poly"));   // 3^3 - general extension field (requires poly representation)
  EXPECT_NO_THROW(GF(49, "poly"));   // 7^2 - general extension field (requires poly representation)
  EXPECT_NO_THROW(GF(125, "poly"));  // 5^3 - general extension field (requires poly representation)
...
TEST_F(GaloisFieldFactoryTest, VariantTypeHandling) {
  ...
  bool is_prime_field = std::visit([](const auto& field) -> bool {
  ...
  bool is_binary_extension = std::visit([](const auto& field) -> bool {
    using FieldType = std::decay_t<decltype(*field)>;
    ...
    return std::is_same_v<FieldType, GaloisFieldBinaryExtension<uint8_t>> ||
           std::is_same_v<FieldType, GaloisFieldBinaryExtension<uint16_t>> ||
           std::is_same_v<FieldType, GaloisFieldBinaryExtension<uint32_t>> ||
