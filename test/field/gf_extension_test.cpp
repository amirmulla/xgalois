...
protected:
  void SetUp() override {
    // Create GF(3^2) = GF(9)
    gf9 = std::make_unique<GaloisFieldExtension<uint8_t>>(
        std::make_pair(3, 2));

    // Create GF(5^2) = GF(25)
    gf25 = std::make_unique<GaloisFieldExtension<uint8_t>>(
        std::make_pair(5, 2));
  }
...
TEST_F(GaloisFieldExtensionTest, InvalidOperations) {
  ...
  EXPECT_THROW(gf9->Div(gf9->MultiplicativeIdentity(), zero), std::domain_error);
...
TEST_F(GaloisFieldExtensionTest, Constructor) {
  ...
  // Test invalid orders
  EXPECT_THROW(GaloisFieldExtension<uint8_t>(6), std::invalid_argument);  // Not a prime power
  EXPECT_THROW(GaloisFieldExtension<uint8_t>(10), std::invalid_argument); // Not a prime power
}
...
TEST_F(GaloisFieldExtensionTest, SetElementValueString) {
  ...
  // Test simple polynomial: "α + 1" should be polynomial with coefficients [1, 1]
...
TEST_F(GaloisFieldExtensionTest, SetElementValueStringInvalid) {
  ...
  EXPECT_THROW(gf9->SetElementValue("β + 1"), std::invalid_argument);  // Wrong variable
  EXPECT_THROW(gf9->SetElementValue("h^1"), std::invalid_argument);    // Wrong generator name for power
  EXPECT_THROW(gf9->SetElementValue("g^"), std::invalid_argument);     // Incomplete power format
}
...
TEST_F(GaloisFieldExtensionTest, SetElementValuePolynomial) {
  ...
  coeffs.push_back(GaloisFieldElementBase<GaloisFieldPrime<uint8_t>>(2, base_field_ptr)); // 2
  coeffs.push_back(GaloisFieldElementBase<GaloisFieldPrime<uint8_t>>(1, base_field_ptr)); // α
...
TEST_F(GaloisFieldExtensionTest, SetElementValueReduction) {
  ...
  std::vector<GaloisFieldElementBase<GaloisFieldPrime<uint8_t>>> coeffs;
  coeffs.push_back(GaloisFieldElementBase<GaloisFieldPrime<uint8_t>>(1, base_field_ptr)); // 1
  coeffs.push_back(GaloisFieldElementBase<GaloisFieldPrime<uint8_t>>(0, base_field_ptr)); // 0*α
  coeffs.push_back(GaloisFieldElementBase<GaloisFieldPrime<uint8_t>>(0, base_field_ptr)); // 0*α^2
  coeffs.push_back(GaloisFieldElementBase<GaloisFieldPrime<uint8_t>>(1, base_field_ptr)); // 1*α^3
...
TEST_F(GaloisFieldExtensionTest, SetElementValuePowerStringInvalid) {
  ...
  EXPECT_THROW(gf9->SetElementValue("h^1"), std::invalid_argument);  // Wrong generator name
}
