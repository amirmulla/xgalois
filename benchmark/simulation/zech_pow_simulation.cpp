...
int main() {
    ...
    std::uniform_int_distribution<uint32_t> exp_dis(0, 1000); // Random exponents
    ...
    std::cout << "Generating " << num_elements << " random field elements and exponents..." << std::endl;
    ...
    std::cout << "Measuring exponentiation of " << num_elements << " random pairs..." << std::endl;
    ...
    double avg_time_ns = static_cast<double>(duration.count()) / log_elements.size();
    ...
    std::cout << "Average time per exponentiation: " << avg_time_ns << " ns" << std::endl;
    ...
    std::cout << "\n=== Performance with Different Exponent Ranges ===" << std::endl;

    std::vector<std::pair<uint32_t, uint32_t>> exp_ranges = {
        {0, 10},      // Small exponents
        {0, 100},     // Medium exponents
        {0, 1000},    // Large exponents
        {0, 10000}    // Very large exponents
    };

    for (const auto& range : exp_ranges) {
        std::uniform_int_distribution<uint32_t> range_exp_dis(range.first, range.second);
