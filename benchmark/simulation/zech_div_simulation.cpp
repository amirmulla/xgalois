...
int main() {
  ...
  std::cout << "Measuring division of " << num_elements << " random pairs..."
            << '\n';
  ...
  for (size_t i = 0; i < log_elements.size() - 1; ++i) {
    ...
    if (divisor == static_cast<uint32_t>(-1)) {
      divisor = log_elements[i];
    }
    if (divisor == static_cast<uint32_t>(-1)) {
      divisor = 0;
    }
  ...
  std::cout << "\nDivision simulation completed successfully!" << '\n';
  return 0;
}
