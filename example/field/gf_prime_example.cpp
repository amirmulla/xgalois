 ...
 * @brief Examples for prime field operations using GFP alias and direct constructors
 */
...
int main() {
    ...
    std::cout << "============================\n";
    std::cout << "This example uses GFP alias and direct constructors (NO factory)\n";
    ...
    std::cout << "  Order: " << gf7_field->Order() << ", Characteristic: " << gf7_field->Characteristic() << "\n";

    auto gf11_field = std::make_shared<GFP<uint8_t>>(11);
    std::cout << "Created GF(11) using GFP<uint8_t> alias\n";
    std::cout << "  Order: " << gf11_field->Order() << ", Characteristic: " << gf11_field->Characteristic() << "\n";

    auto gf101_field = std::make_shared<GFP<uint8_t>>(101);
    std::cout << "Created GF(101) using GFP<uint8_t> alias\n";
    std::cout << "  Order: " << gf101_field->Order() << ", Characteristic: " << gf101_field->Characteristic() << "\n";
    ...
    std::cout << "Elements: a = " << static_cast<int>(a.Value()) << ", b = " << static_cast<int>(b.Value()) << ", c = " << static_cast<int>(c.Value()) << "\n";
    ...
    std::cout << "Verification: a * a^(-1) = " << static_cast<int>((a * a_inv).Value()) << " (should be 1)\n";
    ...
    std::cout << "(a + b) * c - a * b = " << static_cast<int>(complex_expr.Value()) << "\n";
    ...
    for (int i = 1; i < 11; ++i) {
        ...
        auto inv = elem.Inv();
        std::cout << i << "^(-1) = " << static_cast<int>(inv.Value()) << " (check: " << i << " * "
                  << static_cast<int>(inv.Value()) << " = " << static_cast<int>((elem * inv).Value()) << ")\n";
    }
