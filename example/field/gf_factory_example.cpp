 ...
 * @brief Examples demonstrating the GF() factory for creating various Galois
 * fields
 */
...
// Helper function to perform field operations on simple integer fields
// (non-extension)
  ...
  return std::visit(
      [a, b, &op](const auto& field_ptr) -> uint64_t {
        ...
        if constexpr (std::is_same_v<FieldType,
          ...
          // In a real application, you'd use FetchElement or work with
          // polynomials directly
          return 0;  // Placeholder
        } else {
          // For simple fields (prime, binary, binary extension with integer
          // elements)
...
int main() {
  ...
  // Create general extension fields using factory (these use polynomial
  // elements) Extension fields require "poly" representation
