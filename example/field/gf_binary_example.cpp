 ...
 * @brief Examples for binary field operations using GF2, GF2X aliases and direct constructors
 */
...
int main() {
    ...
    std::cout << "=============================\n";
    std::cout << "This example uses GF2, GF2X aliases and direct constructors (NO factory)\n";
    ...
    std::cout << "  Order: " << gf2_field->Order() << ", Characteristic: " << gf2_field->Characteristic() << "\n";
    ...
    std::cout << "  Order: " << gf4_field->Order() << ", Characteristic: " << gf4_field->Characteristic() << "\n";
    ...
    std::cout << "  Order: " << gf8_field->Order() << ", Characteristic: " << gf8_field->Characteristic() << "\n";
    ...
    std::cout << "  Order: " << gf16_field->Order() << ", Characteristic: " << gf16_field->Characteristic() << "\n";
    ...
    std::cout << "  x = " << (int)x << " (binary: " << std::bitset<4>(x) << ", polynomial: x + 1)\n";
    std::cout << "  y = " << (int)y << " (binary: " << std::bitset<4>(y) << ", polynomial: x² + 1)\n";
    std::cout << "  z = " << (int)z << " (binary: " << std::bitset<4>(z) << ", polynomial: x³ + 1)\n";
    ...
    std::cout << "  x * y = " << (int)product << " (binary: " << std::bitset<4>(product) << ")\n";
    std::cout << "  x / y = " << (int)quotient << " (binary: " << std::bitset<4>(quotient) << ")\n";
    std::cout << "  x^(-1) = " << (int)x_inv << " (binary: " << std::bitset<4>(x_inv) << ")\n";

    // Verify multiplicative inverse
    auto verification = gf16_field->Mul(x, x_inv);
    std::cout << "  Verification: x * x^(-1) = " << (int)verification << " (should be 1)\n";

    // Demonstrate field properties
    std::cout << "\n=== Field Properties ===\n";
    std::cout << "GF(2^4) characteristics:\n";
    std::cout << "  Extension degree: " << (int)gf16_field->Degree() << "\n";
    std::cout << "  Primitive element: " << (int)gf16_field->MultiplicativeGenerator() << "\n";
